...
const HINT_NUDGES = {
  ...
  bamboo:    'Slow down at the bends — where is the ring straying off-path?',
...
function triggerCanvasRedraw(screenId) {
  ...
  else if (screenId === 'screen-bamboo') drawBuzzWireScene(ctx, canvas);
...
function enterStation(stationId) {
  ...
  if (stationId === 'bamboo') { BuzzWire.cleared = []; initBuzzWire(); }
...
function _ptSegDistSq(px, py, ax, ay, bx, by) {
  ...
  if (!len2) return (px-ax)**2 + (py-ay)**2;
  const t = Math.max(0, Math.min(1, ((px-ax)*dx + (py-ay)*dy) / len2));
  return (px - ax - t*dx)**2 + (py - ay - t*dy)**2;
}

function checkWireTouch(x, y, W, H) {
  const pts = WIRE_SHAPES[BuzzWire.shapeIdx].pts;
  const thresh = (BuzzWire.ringR + BuzzWire.wireR) ** 2;
  for (let i = 0; i < pts.length - 1; i++) {
    if (_ptSegDistSq(x, y, pts[i][0]*W, pts[i][1]*H, pts[i+1][0]*W, pts[i+1][1]*H) < thresh) return true;
  }
  return false;
}

function getWireProgress(x, y, W, H) {
  const pts = WIRE_SHAPES[BuzzWire.shapeIdx].pts;
  let best = 0, bd = Infinity;
  pts.forEach((p, i) => {
    const d = (x - p[0]*W)**2 + (y - p[1]*H)**2;
    if (d < bd) { bd = d; best = i; }
  });
  return best / (pts.length - 1);
}

function updateBuzzWireUI() {
  const btn = document.getElementById('btn-complete-bamboo');
  if (btn) btn.disabled = BuzzWire.cleared.length < 3;
  WIRE_SHAPES.forEach((s, i) => {
    const el = document.getElementById(`btn-wire-${i}`);
    if (!el) return;
    el.classList.toggle('active', i === BuzzWire.shapeIdx);
    el.textContent = BuzzWire.cleared.includes(i) ? `✓ ${s.name}` : s.name;
  });
}

function initBuzzWire() {
  ...
  canvas.onmousemove = (e) => {
    ...
    if (!BuzzWire.active) {
      ...
      if (getWireProgress(mx, my, W, H) > 0.90 &&
        ...
        const acc = Math.max(0, 100 - BuzzWire.buzzes * 10);
        const passed = acc >= 70;
        if (passed && !BuzzWire.cleared.includes(BuzzWire.shapeIdx)) {
          BuzzWire.cleared.push(BuzzWire.shapeIdx);
        }
        document.getElementById('bamboo-tokens-val').textContent = `${BuzzWire.cleared.length} / 3`;
    ...
    if (BuzzWire.flashTimer > 0) BuzzWire.flashTimer--;
    drawBuzzWireScene(ctx, canvas);
  };
...
function resetBuzzWire() { initBuzzWire(); }

function selectWireShape(idx) {
  Sound.click();
  BuzzWire.shapeIdx = idx;
  initBuzzWire();
}
